  ...
  <body class="bg-gray-50 text-gray-900 flex flex-col min-h-screen">
    ...
    <main class="max-w-6xl mx-auto px-4 py-10 flex-grow">
      ...
      <div class="mb-8 bg-white/80 backdrop-blur rounded-2xl shadow p-4">
        <div class="flex flex-wrap gap-2">
          <a
            ...
            class="px-4 py-2 rounded-full text-sm font-medium {{ ! request('kategori') ? 'bg-emerald-600 text-white shadow' : 'bg-gray-100 hover:bg-gray-200' }}"
          >
            Semua
          </a>
          ...
          @foreach ($semuaKategori as $kategori)
            <a
              href="{{ route('katalog.index', ['kategori' => $kategori]) }}"
              class="px-4 py-2 rounded-full text-sm font-medium {{ request('kategori') == $kategori ? 'bg-emerald-600 text-white shadow' : 'bg-gray-100 hover:bg-gray-200' }}"
            >
              {{ $kategori }}
            </a>
          @endforeach
      ...
      <div class="grid gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
        @forelse ($items as $item)
          <div class="bg-white rounded-2xl overflow-hidden product-card flex flex-col">
            ...
            <div class="p-4 flex flex-col flex-1">
              ...
              <span
                id="status-{{ $item->idbarang }}"
                class="inline-block mt-2 text-xs px-2 py-1 rounded {{ $item->stok > 0 ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}"
              >
              ...
              <div class="mt-auto pt-4 grid grid-cols-2 gap-2">
                ...
                @if ($item->stok > 0)
                  @auth
                    <form action="{{ route('keranjang.store') }}" method="POST">
                      ...
                      <button
                        class="btn-glow bg-emerald-600 hover:bg-emerald-700 text-white text-sm rounded-lg py-2 w-full"
                      >
                        + Keranjang
                      </button>
                    </form>
                ...
                @else
                  <button
                    class="bg-gray-300 text-gray-500 text-sm rounded-lg py-2 cursor-not-allowed"
                    disabled
                  >
                    Habis
                  </button>
        ...
        @empty
          <div class="col-span-full text-center py-16 text-gray-500">Tidak ada produk</div>
        @endforelse
      </div>
