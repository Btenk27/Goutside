    ...
    <main class="max-w-4xl mx-auto px-4 py-8 space-y-6">
      <header class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <div class="flex flex-wrap gap-2">
          <a href="{{ route('admin.produk.index') }}" class="btn-primary text-sm">
      ...
      @if ($errors->any())
        <div
          class="mb-4 rounded-lg bg-red-100 border border-red-300 px-4 py-3 text-sm text-red-700"
        >
      ...
      <div class="card">
        <form
          action="{{ route('admin.produk.update', $produk->idbarang) }}"
          method="POST"
...
{{ old('spesifikasi', $produk->spesifikasi) }}</textarea
            >
          </div>
          ...
          <div class="grid gap-4 md:grid-cols-3">
            <div>
              ...
              <select name="status" required>
                ...
                <option
                  ...
                  {{ old('status', $produk->status) == 'Tidak tersedia' ? 'selected' : '' }}
                >
                  Tidak tersedia
                </option>
              </select>
            ...
            <div>
              ...
              <select name="kategori">
                ...
                <option
                  value="Tenda"
                  {{ old('kategori', $produk->kategori) == 'Tenda' ? 'selected' : '' }}
                >
                  Tenda
                </option>
                <option
                  value="Carrier"
                  {{ old('kategori', $produk->kategori) == 'Carrier' ? 'selected' : '' }}
                >
                  Carrier
                </option>
                <option
                  value="Alat Masak"
                  {{ old('kategori', $produk->kategori) == 'Alat Masak' ? 'selected' : '' }}
                >
                  Alat Masak
                </option>
                <option
                  value="Elektronik"
                  {{ old('kategori', $produk->kategori) == 'Elektronik' ? 'selected' : '' }}
                >
                  Elektronik
                </option>
                <option
                  value="Peralatan Pribadi"
                  {{ old('kategori', $produk->kategori) == 'Peralatan Pribadi' ? 'selected' : '' }}
                >
                  Peralatan Pribadi
                </option>
                <option
                  value="Survival"
                  {{ old('kategori', $produk->kategori) == 'Survival' ? 'selected' : '' }}
                >
                  Survival
                </option>
              </select>
